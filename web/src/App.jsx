...
function ClipPreview({ url, isImage, autoPlay, onEnded }) {
  if (!url) return null;
  if (isImage) {
    return <img key={url} src={url} alt="preview" style={{ width: '100%', flex: 1, minHeight: 0, objectFit: 'contain', background: '#000', display: 'block' }} />;
  }
  return (
    <video
      key={url}
      ...
      autoPlay={autoPlay}
      onEnded={onEnded}
      style={{ width: '100%', flex: 1, minHeight: 0, background: '#000', display: 'block' }}
    />
  );
}
...
function SearchCard({ item, onAdd, adding }) {
  ...
  return (
    <div
      draggable
      onDragStart={(e) => { dragState.fromId = null; e.dataTransfer.effectAllowed = 'copy'; try { e.dataTransfer.setData('application/x-search-item', JSON.stringify(item)); } catch {} }}
      style={{ background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 8, overflow: 'hidden', opacity: adding ? 0.6 : 1 }}
    >
      {isVid
        ? <video muted preload="metadata" src={item.preview_url} style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', display: 'block', pointerEvents: 'none' }} />
        : <img src={item.thumb_url || item.preview_url} alt="" style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', display: 'block', pointerEvents: 'none' }} />
      }
      <div style={{ padding: '5px 7px' }}>
        <div style={{ fontSize: 10, color: C.muted }}>{item.provider} · {Number(item.duration || 0).toFixed(1)}s</div>
        <button
          disabled={adding}
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (adding || clickedRef.current) return; clickedRef.current = true; onAdd(item).finally(() => { clickedRef.current = false; }); }}
...
export default function App() {
  // ── Todos los useState/useRef/useMemo PRIMERO ──
  ...
  const enabledClips = useMemo(() => (timeline.clips || []).filter((c) => c.enabled !== false), [timeline.clips]);
  ...
  const previewIsImage = useMemo(() => {
    ...
    const clip = (timeline.clips || []).find((c) => c.id === [...selectedIds][0]);
    return /\.(jpg|jpeg|png|gif|webp)$/i.test(clip?.path || '');
  }, [selectedIds, timeline.clips]);

  const selectedClip = selectedIds.size === 1
    ? (timeline.clips || []).find((c) => c.id === [...selectedIds][0])
    : null;
  ...
  async function reload() {
    ...
    } catch (e) { toast(`Error cargando: ${e.message}`, 'err'); }
  }
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && hasMore && !busy) triggerSearch(q, searchPage + 1);
    }, { threshold: 0.1 });
    obs.observe(loaderRef.current);
    return () => obs.disconnect();
  }, [hasMore, searchPage, q, busy]);

  // Avanzar al siguiente clip del timeline
  function advanceTimeline() {
    clearTimeout(timelineTimerRef.current);
    setTimelineIdx((idx) => {
      const next = idx + 1;
      if (next >= enabledClips.length) { setTimelinePlay(false); return 0; }
      return next;
    });
  }
  ...
  useEffect(() => {
    ...
    const clip = enabledClips[timelineIdx];
    if (!clip) { setTimelinePlay(false); setTimelineIdx(0); return; }
    setSelectedIds(new Set([clip.id]));
    const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(clip.path || '');
    // Para imágenes usamos timeout con la duración del clip.
    // Para vídeos usamos onEnded (más preciso) + timeout de seguridad por si el vídeo es más largo.
    const safetyMs = (isImage ? Math.max(1, Number(clip.duration || 4)) : Math.max(Number(clip.duration || 4) + 5, 60)) * 1000;
    timelineTimerRef.current = setTimeout(advanceTimeline, safetyMs);
    return () => clearTimeout(timelineTimerRef.current);
  }, [timelinePlay, timelineIdx, enabledClips]);
  ...
  function triggerSearch(value, page = 1) {
    ...
    searchTimerRef.current = setTimeout(async () => {
      ...
      toast('Buscando...');
      try {
        const data = await searchMedia({ q: q2, type: searchType, providers: searchProviders, page, per_page: perPage, orientation: searchOrientation, min_duration: 0, max_duration: 0 });
  ...
  useEffect(() => { if (q.trim().length > 2) triggerSearch(q, 1); }, [q, searchType, searchProviders, searchOrientation]);
  ...
  async function onAdd(item) {
    ...
    try {
      ...
      setTimeline((t) => ({ ...t, clips: [...(t.clips || []), { id: clip.id, name: clip.name, path: clip.path || clip.clip_path || '', start: Number(clip.start || 0), duration: Math.max(1, Number(clip.duration || 4)), enabled: clip.enabled !== false }] }));
  ...
  function selectClip(id, multi) {
    setSelectedIds((prev) => { const next = new Set(multi ? prev : []); if (prev.has(id) && !multi) next.delete(id); else next.add(id); return next; });
  }
  function removeClip(id) { setTimeline((t) => ({ ...t, clips: (t.clips || []).filter((c) => c.id !== id) })); setSelectedIds((prev) => { const n = new Set(prev); n.delete(id); return n; }); }
  function toggleClip(id) { setTimeline((t) => ({ ...t, clips: (t.clips || []).map((c) => c.id === id ? { ...c, enabled: c.enabled === false } : c) })); }
  function moveClip(id, dir) {
    setTimeline((t) => {
      const clips = [...(t.clips || [])]; const i = clips.findIndex((c) => c.id === id); if (i < 0) return t;
      const j = i + dir; if (j < 0 || j >= clips.length) return t;
      [clips[i], clips[j]] = [clips[j], clips[i]]; return { ...t, clips };
    });
  }
  function reorderClips(fromId, toId) {
    setTimeline((t) => {
      const clips = [...(t.clips || [])]; const from = clips.findIndex((c) => c.id === fromId); const to = clips.findIndex((c) => c.id === toId);
      if (from < 0 || to < 0) return t; const [item] = clips.splice(from, 1); clips.splice(to, 0, item); return { ...t, clips };
    });
  }

  function startTimelinePlay() { if (enabledClips.length === 0) return; setTimelineIdx(0); setTimelinePlay(true); }
  function stopTimelinePlay() { clearTimeout(timelineTimerRef.current); setTimelinePlay(false); setTimelineIdx(0); }

  async function onSave() {
    setBusy(true); toast('Guardando...');
    try { await saveTimeline({ clips: timeline.clips || [], script_text: timeline.script_text || '' }); toast('Guardado ✓', 'ok'); }
    catch (e) { toast(`Error: ${e.message}`, 'err'); } finally { setBusy(false); }
  }
  async function onGenAudio() {
    ...
    try {
      ...
      await reload(); toast('Audio listo ✓', 'ok');
    } catch (e) { toast(`Error audio: ${e.message}`, 'err'); } finally { setBusy(false); }
  }
  async function onGenSubs() {
    ...
    try {
      ...
      await reload(); toast('Subtítulos listos ✓', 'ok');
    } catch (e) { toast(`Error subs: ${e.message}`, 'err'); } finally { setBusy(false); }
  }
  async function onRender() {
    ...
    try {
      ...
      await reload(); toast('¡Render completado ✓!', 'ok');
    } catch (e) { toast(`Error render: ${e.message}`, 'err'); } finally { setBusy(false); }
  }
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.topbar}>
        ...
        {canRender && <button style={S.btnOk} onClick={onRender} disabled={busy}>🎬 Render video</button>}
        <div style={{ flex: 1 }} />
        {hasAudio && <span style={{ fontSize: 11, color: C.ok }}>♪ Audio</span>}
        {hasSubs && <span style={{ fontSize: 11, color: '#e7d84f' }}>◉ Subs</span>}
        <span style={{ fontSize: 12, color: status.type === 'ok' ? C.ok : status.type === 'err' ? C.danger : C.muted, marginLeft: 8 }}>{status.msg}</span>
      </div>
      ...
      <div style={S.sidebar}>
        <div style={S.section}>
          ...
          <textarea value={timeline.script_text || ''} onChange={(e) => setTimeline((t) => ({ ...t, script_text: e.target.value }))} placeholder="Escribe o genera el guion..." style={{ ...S.input, minHeight: 90, resize: 'vertical', fontFamily: 'ui-monospace,monospace', fontSize: 11, lineHeight: 1.5 }} />
        </div>
        <div style={S.section}>
          <div style={S.sectionTitle}>Buscar media</div>
          <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
            <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && triggerSearch(q, 1)} placeholder="Buscar clips..." style={S.input} />
            <button style={S.btnAccent} onClick={() => triggerSearch(q, 1)}>↵</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
            <div>
              <label style={S.label}>Tipo</label>
              <select value={searchType} onChange={(e) => setSearchType(e.target.value)} style={{ ...S.input, padding: '4px 6px' }}>
            ...
            <div>
              ...
              <select value={searchProviders} onChange={(e) => setSearchProviders(e.target.value)} style={{ ...S.input, padding: '4px 6px' }}>
            ...
            <div>
              ...
              <select value={searchOrientation} onChange={(e) => setSearchOrientation(e.target.value)} style={{ ...S.input, padding: '4px 6px' }}>
            ...
            <div>
              ...
              <select id="perPageSelect" defaultValue="16" style={{ ...S.input, padding: '4px 6px' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {results.map((r) => <SearchCard key={`${r.provider}-${r.id}`} item={r} onAdd={onAdd} adding={addingId === `${r.provider}-${r.id}`} />)}
          </div>
      ...
      <div style={S.main}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: C.panel, borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          {timelinePlay
            ? <button style={{ ...S.btnDanger, padding: '3px 12px' }} onClick={stopTimelinePlay}>⏹ Detener</button>
            : <button style={{ ...S.btnAccent, padding: '3px 12px' }} onClick={startTimelinePlay} disabled={enabledClips.length === 0}>▶ Reproducir timeline</button>
          }
          {timelinePlay && <span style={{ fontSize: 11, color: C.muted }}>Clip {timelineIdx + 1}/{enabledClips.length} · {enabledClips[timelineIdx]?.name}</span>}
        </div>

        {/* Preview */}
        {previewUrl
          ? <ClipPreview url={previewUrl} isImage={previewIsImage} autoPlay={timelinePlay} onEnded={timelinePlay ? advanceTimeline : undefined} />
          ...
          <div style={{ background: C.panel, borderTop: `1px solid ${C.border}`, padding: '8px 16px', flexShrink: 0 }}>
            ...
            <div style={S.grid2}>
              <div>
                ...
                <input type="number" min={1} step={0.1} style={S.input} value={Number(selectedClip.duration || 4).toFixed(1)}
              ...
              <div>
                ...
                <input type="number" min={0} step={0.1} style={S.input} value={Number(selectedClip.start || 0).toFixed(1)}
      ...
      <div style={S.timeline}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 10px', borderBottom: `1px solid ${C.border}`, background: C.panel, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 11, color: C.muted }}>{(timeline.clips || []).length} clips · {totalSecs.toFixed(1)}s ({fmtTime(totalSecs)}) · habilitados: {enabledSecs.toFixed(1)}s ({fmtTime(enabledSecs)})</span>
          <div style={{ flex: 1 }} />
        ...
        <div style={S.trackRow}>
          ...
          <div style={S.trackLane} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); try { const raw = e.dataTransfer.getData('application/x-search-item'); if (raw) { onAdd(JSON.parse(raw)); return; } } catch {} dragState.fromId = null; }}>
            {(timeline.clips || []).length === 0
              ? <span style={{ color: C.muted, fontSize: 11 }}>Arrastra clips aquí o usa "+ Timeline"</span>
              : (timeline.clips || []).map((clip, i) => <TimelineClip key={clip.id || i} clip={clip} index={i} selected={selectedIds.has(clip.id)} onSelect={selectClip} onRemove={removeClip} onToggle={toggleClip} onMoveLeft={(id) => moveClip(id, -1)} onMoveRight={(id) => moveClip(id, 1)} totalDuration={totalSecs} onDropReorder={reorderClips} />)
            }
          </div>
        </div>
        <div style={S.trackRow}>
          ...
          <div style={S.trackLane}>
            {hasAudio
              ? <div style={{ height: 34, minWidth: 120, background: 'linear-gradient(135deg,#1a3a2a,#152e22)', border: '1px solid #2a5a3a', borderRadius: 6, padding: '4px 10px', fontSize: 11, color: C.ok, display: 'flex', alignItems: 'center' }}>♪ {timeline.audio?.name || 'voice.mp3'}</div>
        ...
        <div style={{ ...S.trackRow, borderBottom: 'none' }}>
          ...
          <div style={S.trackLane}>
            {hasSubs
              ? <div style={{ height: 34, minWidth: 120, background: 'linear-gradient(135deg,#2a2a1a,#222212)', border: '1px solid #5a5a2a', borderRadius: 6, padding: '4px 10px', fontSize: 11, color: '#e7d84f', display: 'flex', alignItems: 'center' }}>◉ {timeline.subtitles?.name || 'subtitles.srt'}</div>
